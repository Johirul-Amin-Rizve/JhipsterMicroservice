...
import com.ailaaj.paf.config.EmbeddedKafka;
...
@WithMockUser
@EmbeddedKafka
class PosIntegrationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/pos-integration-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/pos-integration-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/pos-integration-kafka/unregister"));
                return;
            }
        }
