...
import com.ailaaj.paf.config.KafkaSseConsumer;
import com.ailaaj.paf.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/reporting-kafka")
public class ReportingKafkaResource {

    private final Logger log = LoggerFactory.getLogger(ReportingKafkaResource.class);
    ...
    public ReportingKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
    ...
    @StreamListener(value = KafkaOrderConsumer.CHANNELNAME, copyHeaders = "false")
    public void consume(OrderDto orderDto) {
        long ailaajMedicineId = orderDto.getAilaajMedicineId();
        long medicineId = orderDto.getMedicineId();
    }
}
